...
**Actors.** General administrator (`STUDYUS_ADMIN`): administers the platform and all tenants, with only aggregate cross-tenant insight in F12. Institution administrator (`INSTITUTION_ADMIN`): administers exactly their tenant and approves/rejects teacher memberships there. Academic coordinator: leads academic analysis for an explicitly assigned institution, with aggregate dashboard access only. Teacher: keeps the existing scoped class/intervention experience. Student and parent: receive no new F12 authority.
...
| REQ-F12-05 | accepted | First release is read-only decision support led by an academic coordinator. |
| REQ-F12-06 | accepted | General administration is platform-wide; institutional administration is tenant-scoped; academic coordination is tenant-scoped aggregate analysis. |
...
flowchart LR
  ...
  F12 -->|platform or tenant scope check| F2[F2 authorization + institution domain]
...
| Role | Scope | Allowed F12 action | Explicitly denied |
|---|---|---|---|
| General administrator (`STUDYUS_ADMIN`) | platform | administer tenants and see platform/cross-tenant aggregates; no learner drill-down | raw learner data; tenant-specific data beyond aggregate outputs unless a separate, audited support case is defined |
| Institution administrator (`INSTITUTION_ADMIN`, approved) | exact institution | administer its tenant; approve/reject teacher memberships; read its protected aggregates | other tenants; learner drill-down |
| Academic coordinator (new institution-scoped membership) | exact assigned institution | lead and read protected aggregate dashboard | membership approval; tenant configuration; other tenants; learner drill-down |
| Teacher | active assignment/class | existing F11 class/intervention work | F12 institutional aggregate endpoint in first release |
| Student/Parent | n/a | none | F12 data |
| Other institution admin/coordinator | different institution | none | all F12 data |

The academic coordinator is an **institution membership**, not a new platform-wide role: it avoids granting a user access outside the institution that explicitly assigned them. Its exact schema and invitation/revocation lifecycle remain F12 implementation work and must follow F2's active-membership, audit and fail-closed patterns.
...
- **ADR-F12-00 (accepted):** use three scopes rather than a single generic “admin”: `STUDYUS_ADMIN` administers the platform, `INSTITUTION_ADMIN` administers one tenant and approves teacher memberships, and an institution-scoped Academic Coordinator leads aggregate academic analysis. The coordinator is not a global role and receives neither membership approval authority nor learner access.
...
| Q-F12-03 | Confirm coordinator lifecycle: who invites, revokes and reassigns an Academic Coordinator | blocks coordinator implementation; role/scope is now decided |
