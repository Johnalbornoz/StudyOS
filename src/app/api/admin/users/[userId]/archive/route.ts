...
import { archiveUser, getClerkIdForCanonicalUser, LastAdminProtectionError } from '@/services/user-admin.service';

const Schema = z.object({ reason: z.string().min(1).max(500) });

export async function POST(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  const guard = await guardAdminUsersRoute('admin.users.archive');
  ...
  try {
    await archiveUser(guard.admin.actor.id, userId, targetClerkId, validated.reason);
