...
import { cancelMembership, getStudentIdForSubscription, InvalidMembershipTransitionError } from '@/services/membership-admin.service';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ subscriptionId: string }> }) {
  const guard = await guardAdminUsersRoute('admin.memberships.cancel');
  ...
  try {
    await cancelMembership(guard.admin.actor.id, studentId, validated.reason);
