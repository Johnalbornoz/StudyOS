...
import { suspendMembership, getStudentIdForSubscription, InvalidMembershipTransitionError } from '@/services/membership-admin.service';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ subscriptionId: string }> }) {
  const guard = await guardAdminUsersRoute('admin.memberships.suspend');
  ...
  try {
    await suspendMembership(guard.admin.actor.id, studentId, validated.reason);
