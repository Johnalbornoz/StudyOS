...
## 23. Evidence Idempotency (Phase 2B)

Phase 2A's audit found the single highest-priority integrity gap in the whole cognitive engine: `learning_evidence` had no way to tell a genuine network retry, double-click, or concurrent duplicate request apart from a real second learner action. Phase 2B closes it. Full design/audit trail: `docs/audits/STUDYUS_PHASE_2B_EVIDENCE_IDEMPOTENCY.md`. This section records the resulting architecture so it doesn't drift from what the report describes at the moment it was written.

**Logical operation identity.** Every evidence-producing feature already funnels through exactly one function — `mastery.service.ts::updateMastery` — confirmed by a fresh grep of every `INSERT INTO learning_evidence` call site in the codebase (there is exactly one). A caller that has a stable, transport-retry-invariant domain identifier for the action it's recording (a `quiz_sessions.id`, a `verification_attempts.id`, an activity id minted once at generation time, an occurrence-independent submission token) passes it as `MasteryUpdateInput.identity: { operationType, operationId, conceptId }`. `src/lib/algorithms/evidence-idempotency.ts::buildOperationKey` turns that into a deterministic, opaque, non-PII string — never built from a timestamp or a freshly-generated random value, since either would make every retry of the same action look like a new one.

**Database uniqueness guarantee.** `learning_evidence.operation_key` (nullable text) plus a partial unique index (`WHERE operation_key IS NOT NULL`) is the actual enforcement mechanism — not a preceding `SELECT`. Two concurrent `updateMastery` calls carrying the same key can never both durably apply: the database itself resolves the race. `assessment_results.submission_token` carries the identical shape for the one writer (`exam-result.service.ts`) whose natural domain key (`assessment_occurrences.id`) cannot safely mean "duplicate" — a real exam result may legitimately be corrected/re-entered later, so identity there is a client-minted, per-deliberate-submission token instead, never the occurrence id itself.

**Exactly-once cognitive effect, not just exactly-once evidence.** `updateMastery` claims `operation_key` via the `learning_evidence` INSERT as the first write of one short database transaction that also performs the Mastery mutation, its `mastery_events` audit row, any `learning_debt` upsert, the classified-error insert, and — critically — Concept Knowledge State recalculation (this section's own projector, `recalculateConceptKnowledgeState`, together with the Phase 2.2B Validation Cycle overlay it calls into) *inside the same transaction*, via an additive, optional `client: DbExecutor` parameter threaded through `getActiveMasteryPolicy`, `getTransferScore`, `getMisconceptionCountsForConcept`, and every function in `validation-cycle.service.ts`'s `evaluateValidationLifecycle` call graph — defaulting to the shared pool for every pre-existing, non-transactional caller, so nothing about this engine's behavior changed for any caller that isn't the new atomic path. `recordDecisionEvent` (`MASTERY_UPDATED`, `LEARNING_DEBT_CREATED`, `KNOWLEDGE_STATE_PROJECTED`) also runs on that same client, and — only when explicitly given one — propagates its own failure instead of swallowing it, so a decision-audit failure correctly rolls back the whole operation rather than leaving Mastery committed with a stale or missing audit trail. If ANY step throws, the whole transaction rolls back: there is no state where evidence exists but Mastery doesn't, or Mastery updated but Knowledge State didn't. Product analytics (`track()`) is the one deliberate exception, kept outside this boundary as best-effort telemetry, not cognitive state.

**Duplicate semantics.** A unique-constraint conflict on `operation_key` specifically is not an error — it means this exact operation already applied. `updateMastery` returns `{ ..., duplicate: true, delta: 0 }` reflecting the CURRENT (already-applied) mastery state, never a fabricated one, and never re-invokes Knowledge State recalculation or records a second `MASTERY_UPDATED`/`KNOWLEDGE_STATE_PROJECTED` event. Any other database error still propagates normally.

**Historical boundary.** `operation_key`/`submission_token` are nullable, additive columns; no historical row was backfilled with a fabricated key, and no historical row was deleted or reinterpreted. Postgres treats every `NULL` as distinct under a unique index, so historical evidence — and any writer this phase did not wire an identity for (the generic `/api/learning/record-evidence` route requires its own caller-supplied key instead; see the Phase 2B report's Evidence Writer Audit) — keeps exactly its pre-Phase-2B behavior: read normally, with no special-casing, and with no idempotency guarantee. The guarantee begins from this migration's production deployment forward, not retroactively.

**HTTP delivery vs. cognitive effect.** StudyUs cannot make its own HTTP delivery exactly-once — a request can still reach the server more than once (AT_LEAST_ONCE, irreducibly). What this section's architecture guarantees is EXACTLY-ONCE COGNITIVE EFFECT for a given stable logical operation identity — the distinction Phase 2B's own report states explicitly and by design.

## 24. Misconception Lifecycle (Phase 2C)

Phase 1F and Phase 2A both found the same real false-negative Mastery risk: once recorded, a misconception stayed "active" forever by construction — `student_misconceptions` had no way to represent that a learner had since corrected it. Since a critical misconception's presence is one of `determineMasteryState`'s required, non-compensating gates (`criticalOk`), a learner who genuinely moved past a foundational misconception could remain permanently unable to reach `VALIDATED_MASTERY` on that concept. Phase 2C closes this; Phase 2C-R then closed a real false-positive defect external review found in Phase 2C's own initial resolution scope (see the "Resolution evidence" paragraph below). Full design/audit trail: `docs/audits/STUDYUS_PHASE_2C_MISCONCEPTION_LIFECYCLE.md` and `docs/audits/STUDYUS_PHASE_2C_R_SIGNATURE_SCOPED_RESOLUTION.md`.

**Identity.** `student_misconceptions` already had exactly the right shape for a current-state row — `UNIQUE(student_id, misconception_signature_id)` — so no new "episode" table was introduced. `misconception_signatures(concept_id, misconception_code)` remains the canonical signature identity, unchanged.

**ACTIVE vs. RESOLVED.** Two persisted states only (`status` column, `CHECK (status IN ('ACTIVE','RESOLVED'))`) — no richer workflow state machine, since actual product semantics don't require one. `resolved_at`/`resolved_by_evidence_id` record the most recent resolution; `reactivation_count` records how many times a RESOLVED signature has reappeared. Full transition history (every observation/resolution/reactivation as its own event) lives in `decision_events` (`MISCONCEPTION_RECORDED`/`MISCONCEPTION_RESOLVED`/`MISCONCEPTION_REACTIVATED`), not duplicated as rows in this table.

**Observation.** A genuinely new or recurring detection (`misconception.service.ts::recordStudentMisconception`) always leaves the row `ACTIVE` — whether it's a brand-new signature, a plain recurrence while already `ACTIVE` (occurrence_count increments, status unchanged), or a reactivation of a `RESOLVED` row (occurrence_count still increments, `resolved_at`/`resolved_by_evidence_id` are cleared, `reactivation_count` increments). `occurrence_count` is a lifetime, never-decremented total across all history — current truth belongs to `status`, historical frequency belongs to `occurrence_count`, and these are deliberately different axes.

**Exactly-once observation (reuses Phase 2B, not a second idempotency system).** Classification (AI) still runs before the transaction — acceptable cost, same as grading. Persistence does not: `mastery.service.ts::updateMastery` accepts an optional `misconceptionObservation` and only calls `recordStudentMisconception` inside its own atomic transaction, after its `operation_key` gate has already confirmed a genuinely new logical operation. A transport replay of the triggering Explain & Defend submission never reaches the mutation at all — the evidence-insert gate above it already rejected the retry.

**Resolution evidence.** Deliberately conservative, and built entirely from already-certified concepts — no invented weighted formula (`misconception.service.ts::isMisconceptionResolutionEvidence`): evidence qualifies only when `ai_assistance_type = 'NONE'` (independence, reusing the existing concept) AND either `sourceType = 'EXPLANATION'` with `scorePercent >= mastery_policies.minimum_understanding` (the same threshold Knowledge State's own Understanding dimension uses), or `sourceType = 'SOLO_VERIFICATION'` with a correct outcome (the system's own "prove it independently" check). Ordinary quiz correctness, assisted evidence, and Transfer are deliberately excluded — no existing architecture links a specific quiz question or Transfer prompt to a specific misconception signature, so treating them as resolution evidence would risk exactly the lucky-guess false resolution this design exists to prevent.

**Resolution scope (corrected by Phase 2C-R).** Phase 2C's initial implementation resolved **every** currently-`ACTIVE` signature for the `(student, concept)` on any qualifying evidence — external review correctly found this unsafe: two or more simultaneously-`ACTIVE` signatures on one concept meant evidence that only genuinely addressed one could false-positively resolve both, opening a false-positive path to `VALIDATED_MASTERY`. Phase 2C-R replaced the concept-wide resolver (`resolveActiveMisconceptionsForConcept`) with a signature-scoped primitive (`resolveMisconceptionSignatures`), called with an explicit, bounded set of signature ids. `mastery.service.ts::updateMastery` decides that scope: an explicit scope wins when a caller supplies one (`MasteryUpdateInput.resolvedMisconceptionSignatureIds` — no current writer does, since no generation-time architecture yet binds a response to one candidate signature); otherwise the conservative fallback applies — the single `ACTIVE` signature on that concept resolves **only** when there is exactly one; two or more `ACTIVE` signatures with no explicit scope resolve **none** of them. Ambiguity is never guessed away.

**Anti-circularity.** Resolution never reads `concept_knowledge_state`, `mastery_state`, or `VALIDATED_MASTERY` — only the current evidence application's own already-computed `result`/`sourceType`/independence. A critical misconception blocking `VALIDATED_MASTERY` can therefore always be resolved by qualifying evidence, with no deadlock.

**Transaction integration and ordering.** Misconception observation, then the resolution check (mutually exclusive within one call — a signature just (re)observed cannot, in the same breath, also be evidence the concept is free of it), both run inside `updateMastery`'s existing transaction, using its same `client`, **before** `recalculateConceptKnowledgeState` is invoked. This is what makes the projector never one interaction behind: a resolution lowers `criticalCount` to what the projector reads in that same pass; a new critical observation raises it in that same pass. Any failure after the misconception mutation rolls it back together with the evidence/Mastery/Knowledge State changes in the same transaction — never a partial effect.

**Knowledge State integration.** `knowledge-state.service.ts` itself was not modified — `determineMasteryState`/`determineValidationReadiness`'s `criticalOk` gate logic is unchanged (Phase 2A already certified it as correctly non-compensating). What changed is exclusively the *input* it receives: `misconception.service.ts::getMisconceptionCountsForConcept` now filters to `status = 'ACTIVE'` for `activeCount`/`criticalCount`/`recurringCount` (additively also returning `historicalCount`/`resolvedCount` for callers that want the full picture). A `RESOLVED` critical misconception is no longer counted and can no longer block `VALIDATED_MASTERY`; an `ACTIVE` one still does.

**Current consumers.** `getRecurringMisconceptions` (NBA v2/v3, today-plan, progress-overview "needs attention" surfaces, the Twin's subject-level view) is now `status = 'ACTIVE'`-scoped too, so a resolved misconception stops nagging these surfaces. `cognitive-diagnosis.service.ts` (root-cause diagnosis) was audited and confirmed to read no misconception state at all today, active or historical — nothing to change there. `remediation.service.ts` was audited and confirmed to carry no misconception-specific linkage today, so no stronger post-remediation resolution signal is currently available to reuse. `tutor-strategy.service.ts::buildCompactTutorContext` had a raw, lifecycle-unaware query (`occurrence_count >= 2` with no `status` filter) that predated Phase 2C and was missed by Phase 2C's own consumer audit — found and fixed in Phase 2C-R's fresh audit, now scoped to `status = 'ACTIVE'` like every other consumer.

**Twin / DecisionContext.** `learner-twin/readers.ts::readMisconceptionSummary` and `MisconceptionSummary` (which flows directly into `DecisionContext.misconceptions`) source their counts from the same, now-corrected `getMisconceptionCountsForConcept` — `activeCount`/`criticalCount` mean current state with no further change needed at the Twin layer. `resolvedCount` is additionally exposed; the internal `historicalCount` is deliberately not, keeping the Twin a read layer over current-state-relevant fields rather than a full misconception history browser.

**Historical migration.** Existing rows (none exist in production as of this writing) default to `status = 'ACTIVE'` via the migration's own `DEFAULT` clause — the only epistemically honest choice, since there is no evidence they were ever resolved. `PRE_LIFECYCLE_MISCONCEPTION_STATUS = ASSUMED_ACTIVE_UNTIL_REVALIDATED`.
