...
**[Onboarding rework, 2026-09-21]**: the Parent flow described in this section has changed — a Parent can no longer initiate a link to a student by email (`POST /api/parent/link-child`'s create path was removed); the flow is now student-initiated only (student invites by email → parent accepts with that exact email). The Teacher flow's PENDING/APPROVED/assignment conditions are unchanged, but a self-service UI for institution selection, status, and the coordinator's approve/reject/revoke/assign actions was added where none existed before. Student demo-vs-full-access enforcement was extended to routes this section did not yet cover (tutor, quizzes, cognitive, study-plan). Full detail: [`docs/implementation/f15/F15_ONBOARDING_AUTHORIZATION_REWORK.md`](../implementation/f15/F15_ONBOARDING_AUTHORIZATION_REWORK.md).
