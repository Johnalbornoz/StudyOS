...
**[FASE 0 FREEZE — 2026-09-21]**: "Only ONE hard Pilot gate remains" and "architecturally resolved" (for exam-taking) are both retracted as current-state claims (preserved above, not deleted). A manual attempt on 2026-09-21 executed the exam-taking leg of the pending E2E and found it FAILS on real content (`MINI_MOCK`, `CONCEPT_NOT_MATCHED`, skip-only) — "architecturally resolved" was true of the runner's code and unit tests, not of the resulting user experience against the seeded catalog. This is now `IVG-F15-14`/R13, a second independent hard Pilot gate. Full reconciliation: `docs/implementation/f15/F15_PHASE0_ACCEPTANCE_FREEZE.md`.
