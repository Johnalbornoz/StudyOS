...
- **Timed exams**: `timingMode` (`TRAINING_TIMED`/`OFFICIAL_SIMULATION_TIMED`) is accepted at attempt-start (unchanged from F9/F14) but `ItemRunner` does not enforce or display a countdown — a real, disclosed scope limit. `UNTIMED` attempts (the only mode exercised by this phase's own testing) work fully. **[FASE 0 FREEZE — 2026-09-21]**: "work fully" is retracted as a general claim. It was true for this phase's own unit-level exercise of the runner's mechanics (navigation, submission, skip, completion call), but a real manual attempt against the seeded Pilot catalog (`MINI_MOCK`, `TRAINING_TIMED` shown in the UI) returned only 1 question with `ITEM_UNAVAILABLE`/`CONCEPT_NOT_MATCHED` and no way to complete a meaningful exam — the runner mechanics work, the underlying content/blueprint coverage does not yet support a real attempt. See `docs/implementation/f15/F15_PHASE0_ACCEPTANCE_FREEZE.md`.
