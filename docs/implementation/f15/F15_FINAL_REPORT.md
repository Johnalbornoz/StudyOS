...
**[Onboarding/authorization rework, 2026-09-21]**: a further 7 commits (`6ef8be1`..`a85aeda`, current HEAD `a85aeda4e33c93461b5fc1a761f709f58582b7c7`, same branch) close a set of gaps the task's own inspection surfaced independent of the two hard gates above: the Clerk webhook and dashboard layout no longer silently default every account to STUDENT; a Parent can no longer self-link to a student by email; premium AI/quiz/study-plan routes are now server-side gated by `LEARNING_FULL_ACCESS` where they previously were not; a Teacher self-service institution-selection UI and an institution coordinator approve/reject/revoke/assign UI now exist where before only ungated backend routes did. Full detail, matrix, and residual risks: `F15_ONBOARDING_AUTHORIZATION_REWORK.md`. **This work has NOT been deployed to Preview and its one new migration (`20260921_1000_student_initiated_parent_invitation.sql`) has NOT been applied to any database** — both are the explicit next operator/agent action, not yet done as of this SHA. The Final Decision below (`READY FOR PILOT: NO`, gated on R1/R2) is unchanged and, if anything, reinforced: this phase adds a third, smaller pending item (this migration + deploy) to what must happen before Pilot, on top of the two hard gates already recorded.
