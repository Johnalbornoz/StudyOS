...
The only percentage this document introduces is: **"N of M blueprint objective targets have `SUPPORTED_AND_EVIDENCED` status"**, where `M` excludes `UNSUPPORTED_BY_PLATFORM`/`NOT_REQUIRED`/`UNMAPPED` targets from the denominator — a target the platform cannot even offer, that isn't required, or that has no curriculum mapping at all yet, can never count against the learner (INV-F9-05/08 combined; `UNMAPPED` is excluded for the identical reason as `UNSUPPORTED_BY_PLATFORM` — a missing curriculum mapping is platform/editorial incompleteness, not a fact about learner evidence). Every percentage carries this exact, reproducible denominator in `readiness_snapshots.blueprint_coverage`; there is no other place in F9 that emits a percentage without an equally explicit, named denominator (INV-F9-08, task §8's own requirement).
