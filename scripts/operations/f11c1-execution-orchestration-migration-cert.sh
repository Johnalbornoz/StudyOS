#!/bin/bash
# F11-C1 -- Concept Reinforcement Execution Orchestration certification
...
# F11-C1's own new, purely additive teacher_intervention_executions
# migration, then runs the full authorization + idempotency + lifecycle
# + canonical-evidence certification.
...
WORKDIR="$(mktemp -d /tmp/studyus-f11c1-cert.XXXXXX)"
PGDATA="$WORKDIR/pgdata"
SOCKDIR="$WORKDIR/sock"
LOGFILE="$WORKDIR/postgres.log"
DBNAME="studyus_f11c1_cert"
...
echo "=== F11-C1 Concept Reinforcement Execution Orchestration — local migration + certification ==="
...
echo "--- applying full migration history (chronological, through F11-C1's own new migration) ---"
for f in "$MIGRATIONS_DIR"/*.sql; do
  echo "  applying $(basename "$f")"
  $PSQL -f "$f" >/dev/null
done

echo "--- idempotency check: re-applying the F11-C1 migration a second time ---"
$PSQL -f "$MIGRATIONS_DIR/20260929_1000_f11c1_concept_reinforcement_execution.sql" >/dev/null
echo "  OK -- second application produced no error (idempotent)"

echo "--- verifying target schema objects exist ---"
$PSQL -tAc "SELECT to_regclass('public.teacher_intervention_executions')" | grep -q teacher_intervention_executions
echo "  OK -- teacher_intervention_executions table present"

echo "--- running the F11-C1 execution orchestration certification ---"
DATABASE_URL="postgresql://postgres@localhost/$DBNAME?host=$SOCKDIR" \
  "$TSX" "$REPO_ROOT/scripts/operations/f11c1-execution-orchestration-cert-runner.ts"

echo ""
echo "=== F11-C1 migration + certification: ALL CHECKS PASSED ==="
