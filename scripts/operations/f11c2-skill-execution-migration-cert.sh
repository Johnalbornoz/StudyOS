#!/bin/bash
# F11-C2 -- Skill Reinforcement Execution Orchestration certification
...
# F11-C2's own new, additive migration, then runs the full Skill
# targeting + non-inference + F5 aggregation + adversarial + idempotency
# certification.
...
WORKDIR="$(mktemp -d /tmp/studyus-f11c2-cert.XXXXXX)"
PGDATA="$WORKDIR/pgdata"
SOCKDIR="$WORKDIR/sock"
LOGFILE="$WORKDIR/postgres.log"
DBNAME="studyus_f11c2_cert"
...
echo "=== F11-C2 Skill Reinforcement Execution Orchestration — local migration + certification ==="
...
echo "--- applying full migration history (chronological, through F11-C2's own new migration) ---"
for f in "$MIGRATIONS_DIR"/*.sql; do
  echo "  applying $(basename "$f")"
  $PSQL -f "$f" >/dev/null
done

echo "--- idempotency check: re-applying the F11-C2 migration a second time ---"
$PSQL -f "$MIGRATIONS_DIR/20260930_1000_f11c2_skill_reinforcement_execution.sql" >/dev/null
...
echo "  OK -- quiz_sessions.target_skill_ids and the widened execution_type CHECK are present"

echo "--- confirming F5's SKILL aggregation policy is already seeded (no gap here, unlike mastery_policies) ---"
$PSQL -tAc "SELECT 1 FROM aggregation_policy_versions WHERE dimension = 'SKILL' AND status = 'ACTIVE'" | grep -q 1
echo "  OK -- ACTIVE SKILL aggregation policy present"

echo "--- running the F11-C2 skill execution certification ---"
DATABASE_URL="postgresql://postgres@localhost/$DBNAME?host=$SOCKDIR" \
  "$TSX" "$REPO_ROOT/scripts/operations/f11c2-skill-execution-cert-runner.ts"

echo ""
echo "=== F11-C2 migration + certification: ALL CHECKS PASSED ==="
