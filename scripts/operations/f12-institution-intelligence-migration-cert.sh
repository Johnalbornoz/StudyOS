#!/bin/bash
# F12 -- Institution Intelligence certification against a REAL,
# EPHEMERAL, local-only Postgres instance (never Neon/Preview/
# Production). Applies the full existing migration ledger plus F12's
# own new, additive migration, seeds F9's own real PAA/Cambridge
# fixture, then runs the full Institution Intelligence certification.
...
WORKDIR="$(mktemp -d /tmp/studyus-f12-cert.XXXXXX)"
PGDATA="$WORKDIR/pgdata"
SOCKDIR="$WORKDIR/sock"
LOGFILE="$WORKDIR/postgres.log"
DBNAME="studyus_f12_cert"
...
echo "=== F12 Institution Intelligence — local migration + certification ==="
...
echo "--- applying full migration history (chronological, through F12's own new migration) ---"
for f in "$MIGRATIONS_DIR"/*.sql; do
  echo "  applying $(basename "$f")"
  $PSQL -f "$f" >/dev/null
done

echo "--- idempotency check: re-applying the F12 migration a second time ---"
$PSQL -f "$MIGRATIONS_DIR/20261003_1000_f12_institution_intelligence.sql" >/dev/null
...
echo "--- running the F12 Institution Intelligence certification ---"
DATABASE_URL="postgresql://postgres@localhost/$DBNAME?host=$SOCKDIR" \
  "$TSX" "$REPO_ROOT/scripts/operations/f12-institution-intelligence-cert-runner.ts"

echo ""
echo "=== F12 migration + certification: ALL CHECKS PASSED ==="
